    ...
    <td width="350"><h2>Onlineanfrage</h2><br />
    	<?php
				if(isset($_POST['submit'])){
						$result_user = mysql_query("SELECT * FROM contact
								 WHERE contactID = '1'
								 LIMIT 1"); 
						...
						while(list($name,$value)=each($_POST)) {    //Hier werden alle vom Formular mitgegebenen Variablen mit ihrem Wert ausgelesen und der Nachricht mitgegeben (\n mache einen Zeilenumbruch dazwischen)
						if($value=="keine" || $value==""){}
						else{
						$message.="$name: $value\n";
						}}
						mail($admin,$subject,$message,"From: $email");       //Hier erfolgt der Versand des e-mails
